    ...
    public class MessagesController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Post([FromBody]Activity activity)
        {
            if (activity.Type == ActivityTypes.Message)
            {
                ...
                {
                    message = "Hey I am InstagramBot.I will help you with captions";
        ...
        private async Task<string> GetTagsAsync(Activity activity, ConnectorClient connector)
        {
            var imageAttachment = activity.Attachments?.FirstOrDefault(a => a.ContentType.Contains("image"));
            if (imageAttachment != null)
            {
                using (var stream = await GetImageStream(connector, imageAttachment))
                {
                    return await this.captionService.GetTagsAsync(stream);
                }
            }
            ...
            if (TryParseAnchorTag(activity.Text, out url))
            {
                return await this.captionService.GetTagsAsync(url);
            }

            if (Uri.IsWellFormedUriString(activity.Text, UriKind.Absolute))
            {
                return await this.captionService.GetTagsAsync(activity.Text);
            }
